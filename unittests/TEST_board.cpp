...
TEST_F(BoardTests, TestRotationWithObstructions) {
	...
	EXPECT_TRUE(board.rotatePiece());
	EXPECT_EQ(board.absPiecePositionCol, 4) << "rotates in tight between set bocks space does not move";
	EXPECT_EQ(board.absPiecePositionRow, 7) << "rotates in tight between set bocks space needs to move 2 up";

	board[{6, 6}] = 0;

	board.rotatePiece();
	board.absPiecePositionRow = 6;
	board.absPiecePositionCol = 5;
	EXPECT_TRUE(board.rotatePiece());
	EXPECT_EQ(board.absPiecePositionCol, 6) << "rotates along left boundary needs to move 1 left";
	EXPECT_EQ(board.absPiecePositionRow, 6) << "rotates along left boundary needs to move 2 up";
}